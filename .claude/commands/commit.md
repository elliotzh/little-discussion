...
**审阅完成后，立即更新 `docs-todo/` 下对应的审阅建议文件**（按 CLAUDE.md 的 docs-todo 规则：完整覆写以反映当前状态；如所有问题均已解决则删除该文件）。这确保后续无论用户选择哪条路径，docs-todo 都与文档实际状态一致。
...
3. **重新审阅并更新审阅建议**：对修复过的文件重新执行审阅（至少覆盖修复涉及的维度），然后完整覆写对应的 `docs-todo/` 文件以反映修复后的状态；如所有问题均已解决则删除该文件
...
1. **验证 docs-todo 一致性**：确认每个被修改的 `.md` 文件在 `docs-todo/` 下的审阅建议已反映文档的当前状态（如果审阅后又做了修复，docs-todo 必须已被更新；如果审阅发现零问题，docs-todo 文件应已被删除）。如果不一致，先更新 docs-todo 再继续。
2. **提交**：将所有修改的文件加入暂存区（包括 `.md` 文件和 `docs-todo/` 下被更新/删除的文件），根据修改内容生成合适的 commit message，执行提交
3. **同步到 main**（worktree 场景下必须执行，不可跳过）：
